...
def build_data_loader_from_cfg(
    ...
    if use_rcc:
        ...
        print('RCC masking selected!')
    else:
        mask_generator = MaskingGenerator3D(
            input_size=(img_size[0] // patch_size, img_size[1] // patch_size, img_size[2] // patch_size),
            max_num_voxels=0.5 * n_tokens,
        )
        print('Default block-wise masking selected!')
    ...
    from dinov3.data.datasets.ct_dataset import CT3D

    json_path = "paths_filtered4.json"
    norm_setting = "ct"
    dataset = CT3D(
        json_path = json_path,                           
        spacing = (2.0, 2.0, 2.0),
...
def build_single_res_loader_from_cfg(cfg, model, start_iter):
    loaders = [
...
def do_train(cfg, model, resume=False):
    ...
    # This block initializes the 3D patch embed from 2D
    model.student.backbone.inflate_patch_embed3d_from_2d()
    model.teacher.backbone.inflate_patch_embed3d_from_2d()
    model.model_ema.backbone.inflate_patch_embed3d_from_2d()
    if model.has_gram_teacher:
        model.gram_teacher.backbone.inflate_patch_embed3d_from_2d()
