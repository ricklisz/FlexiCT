...
def build_data_loader_from_cfg(
    ...
    if use_rcc:
        from dinov3.data.masking import RCCMaskingGenerator
        mask_generator = RCCMaskingGenerator(
    ...
    lmdb_root_v3 = os.environ.get("FLEXICT_LMDB_ROOT_V3", "/path/to/lmdb/ct_v3")
    lmdb_root_v2 = os.environ.get("FLEXICT_LMDB_ROOT_V2", "/path/to/lmdb/ct_v2")
    # Example dataset names, not comprehensive
    dataset_names_v3 = [
        "AMOS-2350",
    ...
    lmdbs = (
        [f"{lmdb_root_v3}/{name}.lmdb" for name in dataset_names_v3] +
        [f"{lmdb_root_v2}/{name}.lmdb" for name in dataset_names_v2]
    )

    index_txts = (
        [f"{lmdb_root_v3}/{name}_slices.index.txt" for name in dataset_names_v3] +
        [f"{lmdb_root_v2}/{name}_slices.index.txt" for name in dataset_names_v2]
    )

    if lmdb_root_v3.startswith("/path/to") or lmdb_root_v2.startswith("/path/to"):
        raise ValueError("Set FLEXICT_LMDB_ROOT_V3 and FLEXICT_LMDB_ROOT_V2 before launching 2D CT pretraining.")
...
def build_multi_patchsize_loader_from_cfg(cfg, model, start_iter):
    loaders = [
        build_data_loader_from_cfg(cfg, model, start_iter, override_patch_size=16),
        build_data_loader_from_cfg(cfg, model, start_iter, override_patch_size=8),
    ]
...
def build_multi_resolution_data_loader_from_cfg(
    ...
    for increment, (global_crops_size_i, local_crops_size_i, gram_teacher_crops_size_i) in enumerate(
        ...
        for j, ps in enumerate(patch_sizes):
            ...
            loaders.append(
                build_data_loader_from_cfg(cfg=cfg_i, model=model, start_iter=start_iter, override_patch_size=ps)
            )
            final_loader_ratios.append(loader_ratios[increment] / len(patch_sizes))  # split the resolution ratio across patch sizes
...
def do_train(cfg, model, resume=False):
    ...
    model.init_weights()
    # check_params_finite(model.student, "student")
    # check_params_finite(model.teacher, "teacher")
    ...
    if resume and (last_checkpoint_dir := find_latest_checkpoint(ckpt_dir)):
        ...
        # We start from 0 if using gram loss
    ...
    # Build data loader
    data_loader = build_multi_patchsize_loader_from_cfg(
        cfg=cfg,
        model=model,
        start_iter=start_iter,
    )
    # Uncomment this if you need multi-resolution 
    ...
    print(f"Rank {os.environ.get('RANK')} affinity: {os.sched_getaffinity(0)}")
