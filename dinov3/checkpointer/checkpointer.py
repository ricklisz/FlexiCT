...
def _materialize_meta_tensors(module: torch.nn.Module) -> list[str]:
    """Replace any meta parameters/buffers with CPU tensors."""
    meta_names: list[str] = []
    for submodule_name, submodule in module.named_modules():
        for name, param in list(submodule._parameters.items()):
            if param is None or not getattr(param, "is_meta", False):
                continue
            meta_names.append(f"{submodule_name}.{name}".strip("."))
...
def save_checkpoint(
    ...
    **others: Stateful,
    ...
    if hasattr(dcpsd, "StateDictOptions"):
        # Ensure all tensors are CPU for filesystem writer.
        try:
            state_dict_options = dcpsd.StateDictOptions(cpu_offload=True)
        except TypeError:
            state_dict_options = dcpsd.StateDictOptions(offload_to_cpu=True)
    meta_names = _materialize_meta_tensors(model)
    if meta_names:
        logger.warning(
            "Materialized meta tensors before checkpoint save: %s",
            meta_names,
        )
    if state_dict_options is not None:
        to_save["model"] = dcpsd.get_model_state_dict(
            model, options=state_dict_options
        )
        if optimizer is not None:
            try:
                to_save["optimizer"] = dcpsd.get_optimizer_state_dict(
...
def load_checkpoint(
    ...
    **others: Stateful,
    ...
    # md = reader.read_metadata()                   # loads the .metadata pickle
    # keys = list(md.state_dict_metadata.keys())    # keys match the saved state_dict’s FQNs
    ...
    if not Path(checkpoint_path).is_dir():  # PyTorch standard checkpoint
        ...
        chkpt = {
            k: (
                ...
                if not any(k.startswith(pns) for pns in prefixes_not_sharded)
                else v
            )
...
def init_fsdp_model_from_dinov3(
    model: torch.nn.Module,
    checkpoint_path: str,
    skip_load_prefixes: List[str] | None = None,
    prefixes_not_sharded: List[str] | None = None,
    ...
    if 'teacher' in chkpt.keys():
        ...
        if skip_patch_embed:
            chkpt = {
                k: v
                for k, v in chkpt.items()
                if 'patch_embed_2D' not in k and 'patch_embed_3D' not in k
            }
    ...
    # Handle None case for prefixes_not_sharded
    if prefixes_not_sharded is None:
        prefixes_not_sharded = []
    ...
    # Handle None case for skip_load_prefixes
    if skip_load_prefixes is None:
        skip_load_prefixes = []

    # Skip keys containing any of the substrings
    filtered_chkpt = {
        k: v for k, v in chkpt.items()
        if not any(skip_str in k for skip_str in skip_load_prefixes)
    }

    model.load_state_dict(filtered_chkpt, strict=False)
