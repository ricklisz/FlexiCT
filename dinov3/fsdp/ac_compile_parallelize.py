...
def activation_checkpoint_unet(cfg, model: nn.Module):
    _checkpointing_wrapper = get_activation_checkpoint_wrapper(cfg)
    # Stem
    if hasattr(model, "encoder") and hasattr(model.encoder, "stem"):
        model.encoder.stem = _checkpointing_wrapper(model.encoder.stem)
    # Stages is nn.Sequential in your printout
    stages = model.encoder.stages
    if not isinstance(stages, nn.Sequential):
        raise TypeError(f"Expected model.encoder.stages to be nn.Sequential, got {type(stages)}")

    for i in range(len(stages)):
        stages[i] = _checkpointing_wrapper(stages[i])

    # Optional: proj (cheap but fine to checkpoint)
    model.proj = _checkpointing_wrapper(model.proj)
...
def compile_unet(cfg, model: nn.Module):
    # Stem
    if hasattr(model, "encoder") and hasattr(model.encoder, "stem") and isinstance(model.encoder.stem, nn.Module):
        model.encoder.stem = wrap_compile_block(
            model.encoder.stem, cfg.train.cudagraphs, is_backbone_block=False
        )
    # Encoder stages (your print shows nn.Sequential)
    stages = model.encoder.stages
    if not isinstance(stages, nn.Sequential):
        raise TypeError(f"Expected model.encoder.stages to be nn.Sequential, got {type(stages)}")

    for i in range(len(stages)):
        stages[i] = wrap_compile_block(stages[i], cfg.train.cudagraphs, is_backbone_block=False)

    # Optional: compile projection conv
    if hasattr(model, "proj") and isinstance(model.proj, nn.Module):
        model.proj = wrap_compile_block(model.proj, use_cuda_graphs=False, is_backbone_block=False)
...
def fsdp_unet(fsdp_config: Dict[str, Any], model: nn.Module):
    # Wrap stem first (optional but can help memory)
    if hasattr(model, "encoder") and hasattr(model.encoder, "stem"):
        model.encoder.stem = fully_shard(model.encoder.stem, **fsdp_config, reshard_after_forward=True)

    # Wrap each stage (nn.Sequential)
    stages = model.encoder.stages
    if not isinstance(stages, nn.Sequential):
        raise TypeError(f"Expected model.encoder.stages to be nn.Sequential, got {type(stages)}")

    for i in range(len(stages)):
        stages[i] = fully_shard(stages[i], **fsdp_config, reshard_after_forward=True)

    # Prefetch between consecutive stages (nice-to-have)
    for prev_state, next_state in zip(stages, stages[1:]):
        prev_state: FSDPState
        next_state: FSDPState
        prev_state.set_modules_to_forward_prefetch([next_state])
        next_state.set_modules_to_backward_prefetch([prev_state])

    # Finally shard the whole model (proj/norm/etc)
    fully_shard(model, **fsdp_config, reshard_after_forward=True)
    # Register forward method(s) you will call under FSDP
    register_fsdp_forward_method(model, "forward_features")
...
def ac_compile_parallelize(
    ...
    from dinov3.models.vision_transformer import DinoVisionTransformer, MedDINOv3, FlexiMedDINOv3
    from dinov3.models.unet import ResidualEncoderUNet
    ...
    ARCH_TYPE_MAP = {
        ...
        ResidualEncoderUNet: dict(
            compile_fn=compile_unet,
            fsdp_fn=fsdp_unet,
            activation_checkpointing_fn=activation_checkpoint_unet,
        ),
    }
