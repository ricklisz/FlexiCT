...
def build_model(args, only_teacher=False, img_size=224, device=None):
    if "vit" in args.arch or "meddinov3" in args.arch or "fleximeddinov3" in args.arch:
        vit_kwargs = dict(
    ...
    elif 'ResEncL' in args.arch:
        from . import unet as unets
        ...
        teacher = unets.__dict__[args.arch](**unet_kwargs)
        if only_teacher:
            return teacher, teacher.embed_dim
        student = unets.__dict__[args.arch](
            **unet_kwargs,
        )
        embed_dim = student.embed_dim
...
def build_model_from_cfg(cfg, only_teacher: bool = False):
    if getattr(cfg.student, "img_size", None) is not None:
        img_size = cfg.student.img_size
    else:
