...
from flexi_ct.primus import Primus_Multiscale, Primus_v2
from flexi_ct.models import flexi_ct_backbone_base

_FLEXICT_INTERACTION_INDICES = [3, 7, 11, 15]


def _flexict_segmentation_dim_from_patch_size(patch_size) -> int:
    dim = len(tuple(int(i) for i in patch_size))
    if dim not in (2, 3):
        raise ValueError(f"FlexiCTTrainer only supports 2D and 3D patch sizes, got {patch_size}")
    return dim


def _flexict_checkpoint_for_dim(dim: int) -> str:
    if dim == 2:
        return resolve_flexict_checkpoint("2d")
    if dim == 3:
        return resolve_flexict_checkpoint("3d")
    raise ValueError(f"FlexiCTTrainer only supports dim=2 or dim=3, got {dim}")


def _load_flexict_teacher(backbone: nn.Module, checkpoint_path: str, num_input_channels: int) -> None:
    print(f"Loading Flexi_CT teacher checkpoint from: {checkpoint_path}")
    ...
    if num_input_channels > 1:
        for key in ('patch_embed_2D.proj.weight', 'patch_embed_3D.proj.weight'):
            weight = state_dict.get(key)
            if weight is not None and weight.shape[1] == 1:
                repeats = [1] * weight.ndim
                repeats[1] = num_input_channels
                state_dict[key] = weight.repeat(*repeats) / num_input_channels
...
def _set_flexict_runtime_patch_size(backbone: nn.Module, runtime_patch_size: int) -> None:
    for module in backbone.modules():
        if hasattr(module, 'set_patch_size'):
            module.set_patch_size(runtime_patch_size)
    backbone.patch_size = runtime_patch_size


def _build_flexict_primus(
        patch_size,
        num_input_channels: int,
        num_output_channels: int,
        primus_cls,
        runtime_patch_size: int = 8,
) -> nn.Module:
    dim = _flexict_segmentation_dim_from_patch_size(patch_size)
    model = flexi_ct_backbone_base(in_chans=num_input_channels, patch_size=8,
                                   drop_path_rate=0.2, layerscale_init=1.0e-05, n_storage_tokens=4,
                                   qkv_bias=False, mask_k_bias=True)
    _load_flexict_teacher(model, _flexict_checkpoint_for_dim(dim), num_input_channels)
    if runtime_patch_size != model.patch_size:
        _set_flexict_runtime_patch_size(model, runtime_patch_size)
    return primus_cls(in_chans=num_input_channels, dim=dim, embed_dim=model.embed_dim,
                      patch_embed_size=model.patch_size, num_classes=num_output_channels,
                      dino_encoder=model, interaction_indices=_FLEXICT_INTERACTION_INDICES)


class FlexiCTTrainer(nnUNetTrainer):
    ...
    @staticmethod
    def build_network_architecture(patch_size,
        ...
        return _build_flexict_primus(patch_size, num_input_channels, num_output_channels, Primus_v2)
    ...
    def configure_optimizers(self):
        ...
        def lr_lambda(current_iter):
            ...
            if current_iter < self.warmup_epochs:
                # Linearly increase lr from self.warmup_lr to initial lr for each parameter group.
                # Here, scaling factor is computed relative to self.initial_lr.
                # return (self.warmup_lr + (self.initial_lr - self.warmup_lr) * (current_iter / self.warmup_epochs)) / self.initial_lr
                return float(current_iter + 1) / float(self.warmup_epochs)
            else:
...
class AutoPET_FlexiCTTrainer(FlexiCTTrainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
                 device: torch.device = torch.device('cuda')):
        super().__init__(plans, configuration, fold, dataset_json,  device)
...
class flexict_patch8_multiscale_Trainer(FlexiCTTrainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
                 device: torch.device = torch.device('cuda')):
        super().__init__(plans, configuration, fold, dataset_json,  device)
        self.initial_lr = 5e-5
    ...
    def build_network_architecture(patch_size,
                                   ...
                                   num_output_channels: int,
                                   enable_deep_supervision: bool = True) -> nn.Module:
        return _build_flexict_primus(patch_size, num_input_channels, num_output_channels, Primus_Multiscale)


class flexict_patch8_Primus_v2_Trainer(FlexiCTTrainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
                 device: torch.device = torch.device('cuda')):
        super().__init__(plans, configuration, fold, dataset_json,  device)
        self.initial_lr = 5e-5
...
class flexict_patch16_Primus_v2_Trainer(FlexiCTTrainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
                 device: torch.device = torch.device('cuda')):
        super().__init__(plans, configuration, fold, dataset_json,  device)
        self.initial_lr = 5e-5
    ...
    def build_network_architecture(patch_size,
                                   ...
                                   num_output_channels: int,
                                   enable_deep_supervision: bool = True) -> nn.Module:
        return _build_flexict_primus(patch_size, num_input_channels, num_output_channels, Primus_v2, 16)


class flexict_patch16_multiscale_Trainer(flexict_patch8_multiscale_Trainer):
    @staticmethod
    def build_network_architecture(patch_size,
                                   architecture_class_name: str,
                                   arch_init_kwargs: dict,
                                   ...
                                   num_output_channels: int,
                                   enable_deep_supervision: bool = True) -> nn.Module:
        return _build_flexict_primus(patch_size, num_input_channels, num_output_channels, Primus_Multiscale, 16)
