        ...
        for c in range(1, num_classes+1):
            ...
            if label_c.sum() == 0:
                ...
                try:
                    ...
                    # one of the masks had no object => follow your NaN convention
                    assd_c = np.nan
...
def compute_segmentation_metrics_cardiac_totalseg(
    preds_dir,
    labels_dir,
    class_list,
    file_suffix=".nii.gz",
...
def inference(dataset, trainer):
    preds_dir = f"/path/to/project-data/segmentation_data/nnUNet_results/{dataset}/{trainer}/fold_0/validation"
    csv_filename = preds_dir + "/segmentation_metrics.csv"

    labels_dir = f"/path/to/project-data/segmentation_data/nnUNet_raw_data_base/nnUNet_raw_data/{dataset}/labelsTr"
    # Load dataset.json which contains the labels mapping (including background)
    dataset_json_path = f"/path/to/project-data/segmentation_data/nnUNet_raw_data_base/nnUNet_raw_data/{dataset}/dataset.json"  # Change this path if needed
...
def inference_totalseg_caridac(dataset, trainer):
    ...
    class_list = [
        ...
        {"name": "heart_atrium_right", 'total_id': 48, 'cardiac_id': [16]},
    ]

    print(class_list)
    df, summary_dict = compute_segmentation_metrics_cardiac_totalseg(preds_dir, labels_dir, class_list)
...
if __name__ == "__main__":
    ...
    inference_totalseg_caridac(dataset, trainer)