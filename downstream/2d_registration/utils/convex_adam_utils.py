...
def correlate(mind_fix,mind_mov,disp_hw,grid_sp,shape, ch=12):
    ...
    with torch.no_grad():
        for i in range(disp_hw*2+1):
            ...
            # print('mind sum shape',(mind_fix.permute(1,2,0,3,4)-mind_unfold[:,i:i+H//grid_sp]).shape, 'summed shape',mind_sum.shape)
    ...
    #print(t1-t0,'sec (ssd)')
    #gpu_usage()
    print('final ssd shape',ssd.shape,'final ssd_argmin shape',ssd_argmin.shape)
